...
const Form = () => {
  ...
  const register = async (values, onSubmitProps) => {
    try {
        ...
        reader.onloadend = async () => {
            const fileBuffer = reader.result.split(',')[1];  // Base64-encoded binary string
            const fileName = values.picture.name;
            const fileType = values.picture.type;

            // Upload image to S3
            const uploadResponse = await fetch("http://35.180.228.249:5001/auth/upload", {
                method: "POST",
            ...
            // Register user
            const savedUserResponse = await fetch("http://35.180.228.249:5001/auth/register", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
  ...
  const login = async (values, onSubmitProps) => {
    const loggedInResponse = await fetch("http://35.180.228.249:5001/auth/login", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(values),
    });
